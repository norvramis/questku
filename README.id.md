...
5. Salin kode dari `questku.js` ([raw](questku.js) atau buka di bawah).
...
> [!TIP]
> Kamu juga bisa install sebagai Chrome Extension. Lihat [Instalasi](#instalasi).

---

## Fitur Baru

### Tab HypeSquad — Auto-Select Badge

![tab HypeSquad](assets/tab_HypeSquad.png)

Tab HypeSquad sekarang otomatis mendeteksi dan memilih badge house yang kamu miliki saat tab dibuka.

- **Auto-select** — Saat tab dibuka, badge yang dimiliki otomatis terpilih.
- **Apply / Remove** — Set atau hapus house langsung dari dashboard.
- **Visual feedback** — Warna house diterapkan dinamis (Bravery `#9b59b6`, Brilliance `#e74c3c`, Balance `#1abc9c`).
...
| Queue Manager | Pause/resume, stop, kill. Status terminal nonaktifkan kontrol otomatis. |
| Progress Monitoring | Hover card untuk progress bar. Jumlah antrian aktif terlihat. |
| HypeSquad | Apply atau remove badge house. Auto-detect badge yang dimiliki. |
| Deteksi Nitro Orb | Otomatis mendeteksi Nitro dan memakai nilai orb premium. |
| View Quest | Buka detail quest via SPA routing (tanpa reload, tanpa buka app). |

---

## Cara Kerja

Questku memanfaatkan module loader webpack Discord (`webpackChunkdiscord_app`) untuk mengakses API quest secara langsung — tanpa reverse engineering, tanpa proxy, tanpa server eksternal.

1. **Webpack Discovery** — Menyusup ke `webpackChunkdiscord_app` dan menemukan module Discord (quest, game, streaming, API) dengan mencari signature method tertentu.
2. **Ambil Quest** — Mengambil data quest dari memori Discord (`Q.Quest.quests`) dan memfilter quest aktif yang didukung.
3. **Enrollment** — Mengirim POST request ke API enrollment Discord untuk mendaftarkan user ke quest.
4. **Proses Antrian** — Mengurutkan quest berdasarkan estimasi durasi, lalu memproses satu per satu:
...
Menampilkan semua quest aktif dari API Discord.
...
- View Quest — buka detail quest via SPA routing

### Tab Progress

Manager antrian. Menampilkan quest yang sedang diproses.
...
Pemilihan house dengan deteksi badge yang dimiliki.
...
|-------|--------|-------|-------|
| Info | `[..]` | Oranye | Enrollment, rate limit, event queue |
| OK | `[OK]` | Hijau | Enroll berhasil, claim, selesai |
| Error | `[!!]` | Merah | Request gagal, error quest |
| Header | `[!]` | Biru | Kill, reset, startup |
| Debug | `[DBG]` | Ungu | Diagnostik internal (hanya mode debug) |
...
> [!TIP]
> Aktifkan debug mode dengan设置 `debugMode = true` di bagian atas `questku.js` untuk log yang lebih detail.
...
| Pertanyaan | Jawaban |
|------------|---------|
| Apa yang dilakukan Kill? | Menghentikan semua proses queue, mengembalikan properti module Discord, mengirim event `RUNNING_GAMES_CHANGE`, dan menutup dashboard. |
| Kenapa quest tidak muncul? | Pastikan quest sudah diterima di tab Quests. Hanya quest aktif dengan tipe tugas didukung yang muncul. |
| Bisa menjalankan quest di background? | Bisa. Antrian berjalan independen. Bisa minimize dashboard atau pindah tab. |
