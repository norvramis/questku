(function questku() {
    ...
    let log = {
        ...
        dn(n) { console.log('%c[Done]%c' + n + ',%c' + fmtDur(this._el()), 'color:#43b581;font-weight:bold', '', 'color:#95a5a6') },
    ...
    function getTok() {
        try {
            ...
            if (k) return Q.api[k];
            k = Object.keys(Q.api).find(k => typeof Q.api[k] === 'string' && Q.api[k].length > 40);
            if (k) return Q.api[k];
        } catch {}
        return null;
    }
    ...
    async function directFetch(method, url, body) {
        ...
        try {
            ...
            return { body: text ? JSON.parse(text) : null, status: r.status };
        } catch { return null; }
    }
    ...
    async function hsApply() {
        ...
        try {
            let r = await directFetch('POST', '/hypesquad/online', { house_id: hsState.selected });
            if (!r || r.status >= 400) r = await apiReq('POST', '/hypesquad/online', { house_id: hsState.selected });
            if (r && r.status < 400) {
                hsState.current = r.body?.house_id || hsState.selected;
                hsState.selected = null;
                renderHypeSquad();
            } else {
                D.ap.textContent = 'Failed';
            }
        } catch { D.ap.textContent = 'Error'; }
        hsState.busy = false;
        if (D.ap.textContent === 'Applying...') D.ap.textContent = 'Apply Badge';
        hsUI();
    }
    async function hsRemove() {
        if (hsState.busy) return;
        hsState.busy = true; D.ap.disabled = true; D.rm.disabled = true; D.rm.textContent = 'Removing...';
        try {
            let r;
            try { r = await apiReq('DEL', '/hypesquad/online'); } catch (e) { r = e; }
            if (r && r.status < 400) { hsCurrent(); return; }

            let wreq = webpackChunkdiscord_app.push([[Symbol()], {}, r => r]);
            webpackChunkdiscord_app.pop();
            let chunks = Object.entries(wreq.m);
            let httpApi = null;
            for (let i = 0; i < chunks.length; i++) {
                let [id, fn] = chunks[i];
                if (fn.toString().includes('HTTPUtils')) {
                    let mod = wreq(id);
                    httpApi = Object.values(mod).find(e => e?.get);
                    break;
                }
            }

            if (httpApi?.del) {
                try { r = await httpApi.del({ url: '/hypesquad/online' }); } catch (e) { r = e; }
                if (r?.status < 400) { hsCurrent(); return; }
                if (r?.status === 500) {
                    await sleep(10000);
                    try { r = await httpApi.del({ url: '/hypesquad/online' }); } catch (e) { r = e; }
                    if (r?.status < 400) { hsCurrent(); return; }
                ...
                if (httpApi?.post) {
                    try { r = await httpApi.post({ url: '/hypesquad/online', body: {} }); } catch (e) { r = e; }
                    if (r?.status < 400) { hsCurrent(); return; }
                    try { r = await httpApi.post({ url: '/hypesquad/online', body: { house_id: null } }); } catch (e) { r = e; }
                    if (r?.status < 400) { hsCurrent(); return; }
                }
                if (httpApi?.patch) {
                    try { r = await httpApi.patch({ url: '/hypesquad/online', body: {} }); } catch (e) { r = e; }
                    if (r?.status < 400) { hsCurrent(); return; }
                    try { r = await httpApi.patch({ url: '/hypesquad/online', body: { house_id: null } }); } catch (e) { r = e; }
                    if (r?.status < 400) { hsCurrent(); return; }
                }
            }
            D.rm.textContent = 'Failed';
        } catch (e) { D.rm.textContent = 'Error'; }
        hsState.busy = false;
        if (D.rm.textContent === 'Removing...') D.rm.textContent = 'Remove Badge';
        hsUI();
...
`.trim();
        ...
        D.addq.onclick = async () => {
            ...
            if (unenrolled.length > 0) {
                ...
                let stillUnenrolled = st.allQuests.filter(x => x._sel && !x.userStatus?.completedAt && !x.userStatus?.enrolledAt && !qActive(x));
                let nowEnrolled = st.allQuests.filter(x => x._sel && !x.userStatus?.completedAt && x.userStatus?.enrolledAt && !qActive(x));
    ...
    function renderAllQuests() {
        ...
        for (let q of filtered) {
            ...
            let unsupported = !t;
            let qi = queueItem(q);
            let qTerm = qi && ['done','failed','stopped'].includes(qi.status);
    ...
    function renderProgress() {
        ...
        for (let i = 0; i < filtered.length; i++) {
            ...
            let taskName = TASK_NAMES[t] || t || 'Quest';
            let need = t && cfg?.tasks?.[t]?.target ? cfg.tasks[t].target : 0;
            let isPlayOrStream = (t === 'PLAY_ON_DESKTOP' || t === 'PLAY_ON_XBOX' || t === 'PLAY_ON_PLAYSTATION' || t === 'STREAM_ON_DESKTOP');
            let unit = (t === 'WATCH_VIDEO' || t === 'WATCH_VIDEO_ON_MOBILE') ? 's' : 'min';
            let displayNeed = isPlayOrStream ? Math.ceil(need / 60) : need;
            ...
            html += '<div class="qk-cd' + selCls + '" data-qidx="' + idx + '" data-i="' + idx + '" data-curr="' + (item.curr || 0) + '" data-need="' + need + '" data-display-need="' + displayNeed + '" data-unit="' + unit + '" data-status="' + item.status + '">' +
                ...
                '</div>' +
                '<span class="qk-tg ' + stCls + '">' + stLabel + '</span></div>' +
                '<div class="qk-cd-d"><div class="qk-el"><button class="' + selBtnClsFinal + '" data-i="' + idx + '"' + selDisabled + '>' + selTextFinal + '</button><button class="qk-bb qk-vq" data-i="' + idx + '">' + vqLabel + '</button></div></div>' +
                '</div></div>';
        }
        list.innerHTML = html;
        list.querySelectorAll('.qk-cd[data-qidx]').forEach(cd => {
            cd.addEventListener('mouseenter', () => {
                ...
                let displayNeed = parseInt(cd.dataset.displayNeed) || need;
                ...
                if (txt) {
                    let displayCurr = unit === 'min' ? Math.floor(curr / 60) : curr;
                    if (status === 'done') txt.textContent = displayNeed + ' / ' + displayNeed + ' ' + unit;
                    else if (status === 'failed') txt.textContent = 'Failed';
                    else if (status === 'stopped') txt.textContent = 'Stopped';
                    else if (status === 'paused') txt.textContent = displayCurr + ' / ' + displayNeed + ' ' + unit + ' (Paused)';
                    else txt.textContent = displayCurr + ' / ' + displayNeed + ' ' + unit;
                }
                if (fill) {
...
async function processQueue() {
    ...
    async function setFakeGame(q, pid, appId, exe, appName) {
        let fake = {
            ...
            pid: pid, pidPath: [pid], processName: appName, start: Date.now()
        };
    ...
    function updateProgTick(item) {
        ...
        if (fill) {
            let need = parseInt(cd.dataset.need) || 0;
            let pct = need > 0 ? Math.min(100, Math.round((item.curr || 0) / need * 100)) : 0;
            fill.style.width = pct + '%';
        }
        if (txt && item.status !== 'done' && item.status !== 'failed' && item.status !== 'stopped') {
            let displayNeed = parseInt(cd.dataset.displayNeed) || parseInt(cd.dataset.need) || 0;
            let unit = cd.dataset.unit || 'min';
            let displayCurr = unit === 'min' ? Math.floor((item.curr || 0) / 60) : (item.curr || 0);
            txt.textContent = displayCurr + ' / ' + displayNeed + ' ' + unit;
        }
    }
    ...
    async function processQuest(item) {
        ...
        if (hasApp) {
            const taskConfig = q.config.taskConfigV2 ?? q.config.taskConfig;
            if (taskConfig?.tasks) {
                for (const task of Object.values(taskConfig.tasks)) {
                    if (task.applications?.[0]?.id) {
                        appId = task.applications[0].id;
                        break;
                    }
                }
            }
            if (!appId) appId = q.config.application?.id ?? null;
            if (!appName) appName = q.config.application?.name ?? qName;
        }
        ...
        if (t === 'WATCH_VIDEO' || t === 'WATCH_VIDEO_ON_MOBILE') {
            ...
            try {
                ...
                let app = d?.body?.[0];
                if (!app) {
                    log.e('App not found for ID: ' + appId + ', using quest name');
                }
                let win32exe = app?.executables?.find(x => x.os === 'win32');
                let exe = (win32exe?.name || qName).replace('>', '').replace(/[\\/:*?"<>|]/g, '');
                let g = await setFakeGame(q, pid, appId, exe, qName);
                log.i(exe + ' (PID ' + pid + ')');
