...
#questku-panel .qk-ban-vid{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:0;transition:opacity .2s ease;z-index:1}
    ...
    function renderAllQuests() {
        ...
        for (let q of filtered) {
            ...
            html += '<div class="qk-cd' + selCls + '">' +
                ...
                (logoUrl ? '<div class="qk-game-logo-wrap"><img class="qk-game-logo" src="' + logoUrl + '" loading="lazy" onerror="this.style.display=\'none\'"><span class="qk-promoted">Promoted by <strong>' + (q.config.messages?.gamePublisher || q.config.application?.name || q.config.messages?.gameTitle || 'Quest') + '</strong></span></div>' : '') +
                '</div>' +
...
list.querySelectorAll('.qk-vq').forEach(btn => {
        ...
        list.querySelectorAll('.qk-ban-wrap').forEach(wrap => {
            ...
            if (!vid) return;
            wrap.addEventListener('mouseenter', () => { vid.play(); vid.style.opacity = '1'; if (img) img.style.opacity = '0'; });
            wrap.addEventListener('mouseleave', () => { vid.style.opacity = '0'; if (img) img.style.opacity = '1'; setTimeout(() => vid.pause(), 200); });
        });
